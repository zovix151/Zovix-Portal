"""
FIX: Update generate_emotion_voice() in app.py with 3 fixes:
1. Detailed error catching with exact exception in st.error
2. Devanagari script auto-detection for Hindi voices
3. API key pre-check before each TTS tier
"""
import re
...
if func_start < 0:
    print("ERROR: generate_emotion_voice not found in app.py!")
    exit(1)
...
# Find next top-level def/class or section marker
next_markers = list(re.finditer(r'(?<=\n)(?:def |class |# ===)', rest))
if next_markers:
    func_end = func_start + next_markers[0].start()
else:
...
new_func = '''def generate_emotion_voice(text, emotion="neutral", voice_type="male", output_path=None, elevenlabs_voice_id=None):
    ...
    FIXED:
    - Detailed st.error() with exact exception message  
    - Auto-detect Devanagari (Hindi) script and map to Hindi voices
    - API key pre-check before each tier, alert if all missing
    """
    if not output_path:
    ...
    # --- Helper: log + show exact error in Streamlit ---
    def _fail(msg: str):
        logger.error(msg)
    ...
    if has_devanagari:
        ...
        if not use_hindi_voice:
            use_hindi_voice = True  # Force Hindi voice for Devanagari text

    # ================================================================
    # FIX 3: API KEY PRE-CHECK  
    # Collect available keys so we can alert if none are configured
    # ================================================================
    available_tiers = []
    ...
    if len(available_tiers) <= 1:  # Only fallback
    ...
    if eleven_key and elevenlabs_voice_id:
        try:
            ...
            if resp.status_code == 200 and len(resp.content) > 2000:
                ...
                _fail(f"ElevenLabs: Invalid API key (HTTP {resp.status_code})")
    ...
    if azure_key and azure_region:
        try:
            ...
            if use_hindi_voice:
                ...
                ssml = f'<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="https://www.w3.org/2001/mstts" xml:lang="en-US"><voice name="{voice_name}"><prosody rate="0%" pitch="0%">{text}</prosody></voice></speak>'
            ...
            if resp.status_code == 200 and len(resp.content) > 1000:
                ...
                _fail(f"Azure: Invalid subscription key (HTTP {resp.status_code})")
            elif resp.status_code == 429:
    ...
    try:
        if not os.path.exists(output_path) or os.path.getsize(output_path) < 1000:
            ...
            freq_map = {
                "neutral": 220, "happy": 440, "sad": 180,
                "angry": 330, "excited": 550, "serious": 150, "mysterious": 100,
            }
            amp_map = {
                "neutral": 0.3, "happy": 0.4, "sad": 0.2,
                "angry": 0.5, "excited": 0.5, "serious": 0.25, "mysterious": 0.15,
            }
...
try:
    ...
    print("SUCCESS: app.py compiles OK after replacement!")
    print(f"Function replaced: old={len(old_func)} chars -> new={len(new_func)} chars")
